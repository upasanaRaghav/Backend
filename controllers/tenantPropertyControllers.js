...
export const toggleLikeProperty = async (req, res) => {
  try {
    ...
    const userLikedIndex = property.likes.findIndex((id) => id.equals(userId));
    if (userLikedIndex === -1) {
      property.likes.push(userId);
    } else {
      property.likes.splice(userLikedIndex, 1);
    }

    await property.save();
    res.status(200).json({ message: "Like status toggled", property });
  } catch (error) {
